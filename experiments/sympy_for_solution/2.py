...
# 定义符号变量
x, tau, eta, xi = sp.symbols('x tau eta xi', real=True)
...
# 被积函数: (x+tau+eta+xi) * u^2
u_squared = u**2  # (eta^2 + 2*eta*xi)^2
integrand1 = (x + tau + eta + xi) * u_squared
...
# 外层积分: ∫_{0}^{tau} ... dξ
I1 = sp.integrate(inner_integral1, (xi, 0, tau))
print(f"I1 = {I1}")
print()
...
# 被积函数: (x*tau + eta*xi^2) * u
integrand2 = (x*tau + eta*xi**2) * u
...
# ========== 计算 f(x, tau) ==========
print("计算 f(x, tau)...")
u_xt = x**2 + 2*x*tau  # u(x, tau)
f = u_xt + I1 + I2

# 简化表达式
f_simplified = sp.simplify(f)
print(f"f(x, tau) = {f_simplified}")

# 也可以展开看看
print("\n展开形式:")
f_expanded = sp.expand(f_simplified)
print(f"f(x, tau) = {f_expanded}")