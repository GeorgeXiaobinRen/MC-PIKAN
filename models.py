...
class Unoptd_cPIKANModel(nn.Module):
	def __init__(self, input_dim=1, output_dim=1, hidden_dim=10, degree=3, dtype=torch.float32):
		super(Unoptd_cPIKANModel, self).__init__()
		self.fc1 = kal.unoptd_ChebyKANLayer(input_dim, hidden_dim, degree, dtype)
		self.fc2 = kal.unoptd_ChebyKANLayer(hidden_dim, hidden_dim, degree, dtype)
		self.fc3 = kal.unoptd_ChebyKANLayer(hidden_dim, output_dim, degree, dtype)
